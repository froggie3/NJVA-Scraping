    ...
    def main(self):
        ...
        for i in range(0, tries):
            if not os.path.exists(path=old_json_path):
                if i == 0:
                    print(color('[WARN] %s was not found. Making a copy of \
                        JSON file...' %
        ...
        for thread in thread_list:
            ...
            if not os.path.exists(path=export_path):
                ...
                print(color('[INFO] Exported to %s' %
...
class ThreadsIndexer:
    ...
    def compose_url(self, e: object) -> str:
        return 'https://%s/test/read.cgi/%s/%s/' % (
            e['server'].rstrip(), e['bbs'].rstrip(), e['bbskey'].rstrip()
        )
    ...
    def make_index(self) -> object:
        ...
        while (True):
            ...
            try:
                response: object = requests.get(url=uri, headers={
                    "User-Agent": UserAgent().edge
                })
